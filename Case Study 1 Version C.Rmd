...
#install.packages(c("googleway", "ggrepel", "ggspatial", "libwgeom", "sf", "rnaturalearth", "rnaturalearthdata","theme_bw"))
library(dplyr)
library(caret)
library(ggplot2)
library(tidyverse)
...
# Cleaning & Tidying the Beer dataset with adj. formating - we keep both directories so it works for both in case one need to set the df.
beers <- read.csv ("C:/Users/anhph/OneDrive/Documents/UW PMP Certification/Data Science/SMU/Winter 2019/DS6306 Doing Data Science Course/Week 8/Beer Dataset/Beers.csv", header = TRUE, strip.white=TRUE, na.strings=c("", " ", "NA"))
...
```{r message=FALSE, warning=FALSE}
# Cleaning & Tidying the breweries dataset with adj. formating:
breweries <- read.csv ("C:/Users/anhph/OneDrive/Documents/UW PMP Certification/Data Science/SMU/Winter 2019/DS6306 Doing Data Science Course/Week 8/Beer Dataset/Breweries.csv", header= TRUE, strip.white = TRUE, na.strings=c("", " ", "NA"))
...
# Add readable names to the brewerysState dataframe
...
# Now, we can nudge the counts for MI, HI, FL, and AK a bit so they appear within the state boundaries (blending data)
...
#Q2 : Merge beer data with the breweries data. Print the first 6 observations - first & last
```{r message=FALSE, warning=FALSE}
...
```{r message=FALSE, warning=FALSE}
# Option 1 plot number of NAs using gg_miss_var- we decide to use the gg_miss_var option 2.
#pmiss1 <- gg_miss_var(BeersBreweriesDF)
#pmiss1

# Option2 plot number of NAs using gg_miss_var
pmiss1 <- gg_miss_upset(BeersBreweriesDF, nsets=n_var_miss(BeersBreweriesDF))  
pmiss1

# Based on the missing IBUs by State so we can assess if they are isolated to a few states or are missing across the board
# By Gathering summary numbers by State and plot missing values for IBU and ABV by State
...
# Based on the summary to assess how we need to scale measures, so we can display them on same plot
summary(measMedians)

# Add a column to divide IBU by 1000 so as to plot on same graph
...
# Plot the median values by state & Add 2nd axis for IBU values
...
maxABVtxt <- str_c("  Most Alcoholic Beer/n  State: ", maxABVBeer$Brewery_State, "/n  Beer: ", maxABVBeer$Beer_Name, "/n  ABV: ",maxABVBeer$ABV)
maxIBUtxt <- str_c("  Most Bitter Beer/n  State: ", maxIBUBeer$Brewery_State, "/n  Beer: ", maxIBUBeer$Beer_Name, "/n  IBU: ",maxIBUBeer$IBU)
...
```
# State with Highest IBU is supposed to be MA, but OR has one outliner set it becomes the highest IBU state.
# State with highest ABV is supposed to be KY, but CO has one outliner set it becomes the highest ABV state.
...
# Plot Scatter & Density plots in the same row 
plot_grid(p71, p72, nrow=1, rel_heights=c(1, 1,2), align='h', axis='lr')
```
#p-value < 2.2e-16 thus, we Reject the null hypothesis  - true correlation is equal to 0
...
# Summary the subset data to assess variables
summary(subsetBeers) # 924 obs

# ABV and IBU are on different scales; normalize them & create a simple normalize function
...
p2 + annotate("text", x = xbest, y = ybest + 0.001, label=maxtext, size=3.5) +
...
# Import US State population and area statistics for brewery adequacy analysis
stPopArea <- read.csv ("C:/Users/anhph/OneDrive/Documents/UW PMP Certification/Data Science/SMU/Winter 2019/DS6306 Doing Data Science Course/Week 8/Beer Dataset/StatePopArea.csv", header = TRUE, strip.white=TRUE, na.strings=c("", " ", "NA"))

# Add a column for weighted precent with 75% for population & 25% for area
